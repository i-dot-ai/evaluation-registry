...
def test_create_view(client):
    ...
    with patch.object(share_views, "choose_evaluation_status_view", return_value=HttpResponse()) as mock_status_view:
...
@pytest.mark.django_db
def test_share_view(client, basic_evaluation):
...
@pytest.mark.django_db
def test_share_view_with_conditions(client, impact_evaluation):
    with patch.object(share_views, "evaluation_type_view", return_value=HttpResponse()) as mock_type_view:
